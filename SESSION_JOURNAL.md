...
Builds 325/326 green.

### BUILD 328: coarse to fine in resolution (the speed that pays for 326)

The first 15 % of rounds (the far-field warm-up span, inside the 20 % the trainer already low-passes with frequencyBlurStartVariance) render and are supervised at half the long edge (720 -> 360): a quarter of the pixels, and every Gaussian's footprint shrinks with them, so a step costs about a third. A second TrainerSupervisionBuilder (decoding at 360, its own prefetch worker and frame cache) serves those iterations; the loop takes each frame from the pair its iteration belongs to, the render buffers follow the frame's size exactly as they always did, and at the phase end the coarse cache is dropped. What stays in the photograph's units: the coarse-to-fine blur is scaled by the square of the render scale; the 3D-filter sweep at iteration 500 uses the full-size builder's intrinsics and grid (a sampling rate is a property of the camera the model is viewed through; the full builder decodes one frame at slice start so its intrinsics exist); the densification score is a ranking, so pixel units do not enter it. Every held-out evaluation (first at 800) and every calibration window (moved to 700-729) runs at full size. NOT bit-identical to before (the early steps see different pixels), so this is the one change in the campaign that could move quality; the census records coarseSteps and the long edge, and `coarseResolutionFraction = 0` switches it off. Expected: about 675 x 8 ms saved on a 4,500-round run, roughly the cost 326 added.

Build 327 green (push run of 328).

### BUILD 330: camera pose refinement at ten times the rate, gated by an on-device descent check

The 292 and 320 censuses both show the per-frame camera corrections at a median 0.01 cm and a maximum 0.13 cm over a whole run at lr 1e-5: the refinement was off in all but name, while the pose graph's own residual is ~3 cm (about 9 px at 720 wide from 2 m). Rates go to 1e-4, the per-step clamps (0.0005 rad, 2 mm) and the 0.9 deg / 6.3 cm ceiling stay. Because the gradient's sign and frame conventions have never been exercised at a rate that moves anything, the full rate is used only after a check: at the first profiled step past warm-up and the coarse phase (875 on a 4,500 run), after the step's own Adam update, the frame is rendered twice more on that model, at its current correction and at the correction plus one full-rate step, with the photometric (L1 + SSIM) loss compared. Descent -> full rate for the rest of the run; otherwise the rate stays at the inert 0.1x. The check restores lossAccum and the per-interval stats; everything else it touches is rebuilt by the next step. Census: poseCheckLossBefore/After, poseCheckDescends (-1 never ran). The camera-delta census lines (median/max/common) then show what the refinement did; a common-mode drift would be the thing to watch (held-out cameras do not move).

Builds 329/330 green.

### BUILD 330 MEASURED (owner's diags): 59 s training + 4.8 s pre-pass = 63.8 s at 4,500 rounds (~70/s); held-out 19.54 / SSIM 0.6265 (320: 19.29 / 0.6227), trained-view 22.32 (21.53). Owner: "NOT IMPRESSED", then "HIGHER QUALITY, FASTER PROCESSING."

What the census says: gpuStep 11.19 ms/iter average over 4,500; the stage profile at full size sums to 12.2 ms (backward 7.39, forward 1.65, losses 1.13, optimiser 1.46 (was 0.71: degree 2), sort 0.58); the 675 half-size steps therefore cost about 3.7 ms each (3.4x cheaper) and the first full-size score after them, 18.16 at 800, equalled 320's 18.19: half-size training lost nothing measurable. POSE CHECK 0.10539 -> 0.10516: descent, refinement on; deltas median 0.08 cm, max 0.43 cm, common 0.00 (was 0.01/0.13 at the inert rate). Calibrations at full size: forward2 used (0.94), fused blur used (0.90), SIMD scatter used on the legacy path (0.59 vs 0.67), splat-order and two-pixel backward rejected again. The loop waited 1.1 s for supervision (first visits of each level's frames). The curve still rises at the end (19.51 at 4,000, 19.54 at 4,400).

### BUILD 332: three resolution levels with background preloading; SH optimiser walks only active coefficients; pose rate 3x; the held-out score with cameras aligned

Speed: levels [0.5 until 30 %, 0.75 until 55 %, full after]: the code from 328 generalised to N builders, N prefetch workers, N frame caches; a level's frames are decoded in the background (`TrainerSupervisionPreload`, one frame at a time on a utility queue; the loop joins it before that level's first prefetch) while the level before it trains, and a level's cache is released when its phase ends. Level 0's preload overlaps the seed load. `trainer_adam_sh` walks only `activeSHCoeffCount` coefficients (the same ramp value cameraUniforms sends the rasteriser, now from one shared function): a never-active coefficient has zero gradient and zero moments, so the walk stored back what it read; exact, and up to 9x less optimiser traffic during the first 35 %. Calibration windows moved to 2,500-2,529 (past the last level on a 4,500-round run). Expected: about 1,350 x 3.7 + 1,125 x 7 + 2,025 x 12.5 ms = 38 s of GPU against 50, so about 50 s total.

Quality: camera rates 1e-4 -> 3e-4 (clamps unchanged; the check gates the full rate as before, now at the first profiled step past warm-up whatever the size). And the held-out score is ALSO reported with each held-out camera aligned to the finished model: exposure fitted first (the score's own closed form), then eight clamped gradient steps on the camera only (the backward runs for cameraGrad; stats restored; nothing of the model written), the correction with the lowest photometric loss kept, then the frames scored again: `heldOutPSNRPoseAligned` / `heldOutSSIMPoseAligned` beside the raw and exposure-fitted numbers, never instead of them. The trained frames' poses are refined during the run while held-out frames keep the pre-pass pose, so part of the raw gap is registration; this is the number with that part removed.

Build 332's PR run FAILED: TrainerShaders.metal static_assert `sizeof(TrainerAdamUniforms) == 64` (the struct grew to 68 for activeSHCoeffCount; the Swift-side verify() was updated, the Metal assert was not). The push run 331 passed only because it does not archive.

### BUILD 334: the static_assert, and the full builder's preload joined before every evaluation

The full-size builder's background preload starts when the last level begins (iteration 1,350 on 4,500) and could still be inside the builder when the early-stop evaluation at 1,600 builds its held-out frames on the same builder from the loop thread. A builder is single-threaded by contract, so the loop now joins that preload before every evaluation (the wait is at most the remaining frames, once).

### BUILD 336: the crash past 1,350 (a cache emptied under its own preload), and the pose-aligned score measured against the right photo

Build 334 died "around 1,300 rounds". Cause, read in the code: the memory poll (every 50 iterations) releases the full-size builder's frame cache before it considers any cut, and build 332 had put a background preload INSIDE that builder from the moment the last level begins (iteration 1,350 on 4,500). The release drained the prefetch worker but not the preload, so `frameCache.removeAll()` ran on the loop thread while `rememberFrame` was inserting on the preload's queue: a Swift dictionary written from two threads. 1,400 is the first poll at which two whole caches are live (level 1's plus the full one filling), which is when the footprint first crossed a gate this run. Fix: the release quiesces every worker (prefetches drained, all preloads cancelled and joined) and drops EVERY builder's cache, not just the full one. The census now records the peak footprint the poll saw (`memoryFootprintPeakMegabytes`) so the next diagnostics say how close to the 1.46 GB ceiling a run comes.

Also from the build-332 review, each verified in the code before changing it:
- `alignHeldOutPoses` never uploaded the held-out photo; slot 0 still held the last TRAINING frame, so every loss and camera gradient of the alignment was against the wrong picture. The 334 pose-aligned score is meaningless; 336 uploads the photo first.
- `trainer_adam_sh` walked `activeSHCoeffCount * 3` floats, but the rasteriser and preprocess_backward switch coefficients on a DEGREE at a time (1, 4, 9), so during the ramp the degree's tail coefficients carried real gradients the optimiser skipped (roughly 1,180 of the first 1,575 iterations). Now degree-bounded: exact.
- A preload cancelled before it started stayed cancelled for ever (any governor change killed every later level's preload). Cancellation is cleared on start; a preload does not start once the cache is off.
- Coarse-level cache hits were summed twice; the aligned score is only recorded when the second evaluation scored something; `lastHeldOutSSIM` is cleared on entry like its siblings; with the levels off the full builder's preload no longer starts at iteration 0; SmartAuthorityMap's cache insert re-checks under the lock (two builders now map frames at once).

### BUILD 338: the background preload is off

Build 336 still crashed, now at random points (round 20, round 200, about 1,300). Every reported point falls inside a window where a TrainerSupervisionPreload was decoding on one builder while the active level's prefetch worker built on another (level 1's preload runs from round 0 to about 700, the full builder's from 1,350 to about 1,800); build 330 never had two builders building at once and completed every run. No crash log, so the exact shared state is not identified; the concurrency is removed instead. `TrainerTuning.supervisionPreload = false`: no preload starts, the joins are no-ops, each level decodes its frames on first visit as build 330 did (about 2.7 s for level 1 and 4 s for the full level, on the loop). The three-level schedule, the degree-bounded Adam walk and the 336 fixes stay.

Build 338 RESULT (diagnostics 2026-09-11 11:57Z): 4,500 of 4,500, no crash, no governor cut. Train 52 s + pre-pass 4.6 s = 56.6 s total (330: 63.8 s; 320: 52.6 s). Held-out fitted PSNR 19.53 / SSIM 0.6218 (330: 19.54 / 0.6265), cameras aligned 20.04 / 0.6487 (first honest reading of that number: 336 fixed the missing photo upload). Memory footprint peaked at 1,041 MB against a 1,463 MB ceiling, so the 334/336 crash was not the governor and not memory: with the preload off nothing else changed, which leaves two builders building at once as the cause. GPU-bound: gpuWait 41.3 s of 52; gpuStep 9.5 ms/iter averaged over 2,475 coarse + 2,025 full steps. Frame cache 282 MB, 4,177 hits; preload 0; pose check passed (0.0990 -> 0.0979), full-rate refinement on. Held-out curve starts at 2,800 (every eval inside the coarse levels is skipped on the size mismatch), so early stop had no data before then.
...
### BUILD 346: the memory governor may freeze growth, never delete the model; 150 frames

344 (200 frames, cap 400k) crossed the memory SHARE gate at iteration 200 (footprint 1,174 MB against 0.6 x 1.85 GB free = 1.11 GB), dropped the frame caches, and then cut the splat cap below the live population every 50 iterations down to 20,000; the footprint kept rising to 1,680 MB with the caches gone (each iteration re-decoded its photo), the loop became decode-bound (supervision 8.5 s of 12) and the run ended with 20,000 points at 16.5 dB. 338 had peaked at 1,041 MB, 70 MB under the same gate. Changes: memoryUseFraction 0.6 -> 0.75 (trip at about 1.39 GB on 1.85 GB free; the headroom-floor gate is the jetsam guard and is untouched); the share and ceiling gates now stop at the live population (growth freezes) and return nothing further, and only the floor gate may cut into live points; room keyframes 200 -> 150, cache cap 470 MB. Estimated full-level footprint about 1.16 GB.

### BUILD 350: back to 338's tuning, keeping the governor fix

348 RESULT (340+342+344+346 stacked): 4,500 of 4,500, no cut, 400k points, 135 frames, 54 s train + 4.6 = 58.6 s; held-out fitted 18.69 / SSIM 0.5976, aligned 19.06 / 0.6191 (338: 19.53 / 0.6218, aligned 20.04 / 0.6487; the held-out set changed with the frame count, so PSNR is not like for like, but SSIM fell 0.024 and the run was 2 s slower). Footprint peak 1,054 MB of a 1,480 ceiling. Prefetch worker time 26.6 s (338: 2.9 s) and the loop waited 5.1 s for supervision (338: 1.3 s): unexplained, SmartDepthCache is a plain locked LRU so sharing it is not the cause; the frame count, cap and schedule are the candidates. The stacked experiment cannot be attributed, so 350 reverts every tuning change of 340/342/346 (frames 120, cap 330k, levels 0.30/0.55, cache cap 420 MB) and keeps the code fixes: soft memory gates freeze growth instead of deleting points, memoryUseFraction 0.75, one shared depth cache. Expected: 338's numbers, without the 20,000-point failure mode.

### BUILD 352: towards the Scaniverse reference (Four Marks.ply)

Measured against the owner's Scaniverse export of the same room (427,710 points, SH degree 3, 106 MB): their median long axis is 3.4 mm against our 7.1 mm; 1 % of their points are under 0.1 opacity against 8 % of ours; their SH rest coefficients are 6x ours in magnitude; and their file carries the far field as a shell of points about 240 m out, where our PLY had nothing at all (the cubemap in model/background.bin is read by no viewer, not even the app's). Three changes on top of 350: pruneOpacity 0.02 -> 0.08 (frees the faint 8 % for splits), splitScaleFraction 0.004 -> 0.002 (points over about 1 cm are split, was 2 cm), and the PLY export appends a 24,576-point opaque shell at 50 extents coloured from the frozen cubemap (model count and bounds in model.json stay the model's). SH degree 3 is the next lever and costs optimiser time and about 70 MB.

### BUILD 356: export folds in half the 3D filter

354 RESULT: 4,500 of 4,500, 51 s + 4.6 = 55.6 s, held-out 19.52 / 0.6208, aligned 20.04 / 0.6472, footprint 1,027 MB: the same numbers as 338 to the second decimal. Pruning at 0.08 removed 23,467 and the splits used the room (151k by split), yet the exported median long axis is still 7.1 mm, and 2.5 % of points are under 0.1 opacity. The shell is in the file (353,062 points, 24,576 of them shell). Why the size did not move: the exported sizes carry the mip-splatting 3D filter of the 720-px training grid, which floors every point at about one training pixel, and the training pixel is about twice a viewer's; 3.4 mm at Scaniverse's typical distance is under a 720-px pixel. So the export now folds in half the filter width (a 1,440-px render's), leaving training untouched: `TrainerTuning.exportFilter3DScale = 0.5`. Not measured by any census number (the held-out score is taken before export); judged in a viewer.

### BUILD 360: the last fifth of the run at 1,080 px

358 RESULT: 50 s + 4.6, held-out 19.55 / 0.621, aligned 20.05 / 0.647 (338 again), and the exported median long axis is STILL 7.1 mm with half the filter folded in (p10 3.2, p90 16.1, min axis 0.97 mm). So the size is learned, not the filter: a point trained against 720-px pixels settles at about one of them, roughly 7 mm at room distance, and no split or prune setting changes that. Scaniverse's 3.4 mm is a higher training resolution. Room render long edge 720 -> 1,080 (photos are 1,920 wide); levels 360 px to 45 %, 720 px to 80 %, 1,080 px for the last 20 % (900 steps). Expected GPU: 2,025 x 3.7 + 1,575 x 12.5 + 900 x ~25 ms = about 50 s, so around 60 s total, for points that can settle at about 4.5 mm. Frame cache at 1,080: about 390 MB of the 420 MB cap; footprint estimate 1,240 MB against a 1,390 MB trip. exportFilter3DScale 0.5 -> 0.75 now that the training pixel is smaller.
...
### BUILD 378: resets off, half-size frame cache, scratch aliased, ceiling 60 %

376 RESULT (first run with the memory curve): footprint flat at 1.0-1.25 GB for 24,000 iterations (no leak; whatever 368's 2.8 GB was, the pools or the curve do not show it), then 1,688 MB at 24,050 on entering the 1,080-px level, over the 1,462 MB ceiling (available/2): caches dropped and disabled, the 1,080 phase ran decode-bound, and the opacity reset at 24,000 followed by the prune at 24,500 took the population 330k -> 131k and the score 15.9 -> 14.1 dB; stopped early at 26,800. Worse: the first 1,080-px eval at 24,000 was 15.9 dB, under build 338's 19.5 at 4,500, so the reset recipe hurt long before that (our relocation treats every reset point as a donor). Changes: opacityResetIntervalIterations 0; room cap 300,000; memoryForSplats (ceiling) = 60 % of the allowance (was 50 %); the frame cache stores luma plus 4:2:0 chroma (half the photo bytes: 1,080 px frames 2.6 -> 1.3 MB, cache 390 -> 240 MB at 120 frames; luma exact, chroma averaged over 2x2); densifyScratch is shGrad (63 MB). Periodic census writes no longer double the completed count. Expected 1,080-level footprint about 1.4 GB.

### BUILD 380: the cache stays unless the OS is short; 30 % at 1,080 px; patience 8; cap 330k

378 RESULT: 5 m 16 s, held-out fitted 20.83 / SSIM 0.653, cameras aligned 21.76 / 0.708, trained views 26.67; the owner: 'actually a decent scan', a little under Scaniverse. Memory curve peaked at 1.10 GB in the samples (1.42 at a poll), and the ceiling still tripped once at 24,050 (freeze 300,000 -> 299,940) which dropped the frame cache for the whole 1,080-px phase: 86.7 s of decoding on the worker, and no growth at 1,080 px, so the exported median long axis is 8.5 mm (p10 3.3, p90 24.5). Early stop at 25,600 on a curve rising 0.01 dB an eval. Changes: the caches are released only on the headroom-floor gate (the OS's number); soft gates freeze growth and nothing else. Levels 360 px to 40 %, 720 px to 70 %, 1,080 px for the last 30 % (9,000 iterations). earlyStopPatienceEvals 4 -> 8. Cap back to 330,000 (1.35 GB estimate with the cache, ceiling 1.46, share 1.57).

### BUILD 382: no frame cache for the 1,080-px builder (workaround), memory sampled every 100

380 RESULT: flat at 0.9-1.0 GB through the 360 and 720 levels with their caches (222 MB) serving hits for 9,000 iterations, held-out 20.87 at 21,200; then, with the 1,080-px cache kept (380's policy), 2.6 GB by 21,500 and 2.9 GB by 22,500 while the trainer buffers SHRANK (598 -> 162 MB) and the caches read 0: the OS headroom gate cut the model to 20,000 and the resolution to 384. The growth is about 3.5 MB an iteration (a 1,080 x 810 photo) and only ever appears while the full builder's cache is alive (376: +520 MB in the 50 cached iterations; 378: +320 MB in 50, then flat for 1,600 uncached iterations at 1.1 GB). Not the YCbCr code (376 was RGB), not the coarse caches (flat for 21,000 iterations). Not found; worked around: the full builder's cache is disabled at slice start when levels are on, so the last level decodes on the worker as 378 did. Memory samples every 100 iterations to see the shape next time.

### BUILD 384: cap 400,000 and 200 frames

382 RESULT: 5 m 53 s, 24,400 (stopped early on a flat curve), no cuts, footprint flat 1.03-1.06 GB through the uncached 1,080-px phase (the workaround holds; the growth is tied to the full builder's cache). Held-out 20.87 / 0.663 (best SSIM), aligned 21.37 / 0.688, trained 26.85. Exported median long axis still 8.0 mm: the population sat at the 330k cap from before the 1,080 level, so no split ever happened there. Room cap 330k -> 400k (about +150 MB of buffers: 1.2 GB estimate against a 1.47 GB ceiling) and 200 training frames (720-px caches about 370 MB at 1.85 MB a frame; 1080 is uncached). The held-out set changes with the frame count, so the next PSNR is not like for like.

### 384 RESULT and where the memory goes (no build; the weekly allowance is at 97 %)

384 (200 frames, cap 400k) was killed for memory at level 0 (360 px). The 100-iteration curve is STEPWISE: flat between, then +538 MB at the poll after iteration 1,000, +566 after 1,500, +254 after 2,000, +267 after 2,500 (the last two after the OS gate had cut the population to 152k). Those are the 3D-filter sweeps (filter3DIntervalIterations 500): densify passes (every 100), snapshots (200) and evals (400) show no step. The step is roughly 1.4 KB per live point per sweep, about one bytes-per-splat, and it persists after the sweep. 382 (108 frames, 330k) ran 48 sweeps with a flat curve, and 378's sweeps at 24,500 and 25,000 were flat once the cache was gone, while 380's two sweeps at 21,000 and 21,500 cost 1.6 GB with the 1,080 cache alive. So the sweep leaks per point only in some configurations; the sweep itself (updateFilter3DInner: a clear, one command buffer per camera up to about 90-108, each waited, then a finalise) allocates nothing on the CPU. Untested next step: encode ten cameras per command buffer with one pool per batch and cut the sweep cadence to 2,000, then rerun the 200-frame/400k configuration and read the 100-iteration curve. The working build is 382 (120 frames, 330k, 1,080-px level uncached): 5 m 53 s, held-out 20.87 / 0.663, aligned 21.37 / 0.688.
...
### BUILD 392: memory events inside the loop

390 RESULT (CPU sweep, 200 frames, 400k): the same steps as 384 at the same places (+516 at 1,100, +570 at 1,600, +121 at 2,100, +179 at 2,600; the OS-available column confirms them), so the 3D-filter sweep is NOT the cause (it ran on the CPU with no submissions). The 500-cadence event is the free-space CARVE (carveIntervalIterations 500): the only passes that delete points at this stage of the run, so the only ones that run the gather/compaction; each costs about one bytes-per-splat per live point (516-570 MB at 400k, 121-179 at 106k) and it persists. Growth passes at 300-500 (which also gather) did not step, and 382's carves at 330k / 108 frames did not either; the mechanism is still unknown (resizeSplatCapacity is guarded and the capacity column never changed). This build records iOS's available figure at densify start / after the CPU work / after the gather / end, sweep start / end, eval start / end (`memoryEvents`; census.py prints the steps), so the next run names the stage.

### BUILD 396: back to 120 frames / 330k; what the memory events showed

394 RESULT (200 frames, 400k, CPU sweep, events at every stage): every densify pass, sweep and eval is FLAT from start to end (afterCPU, afterGather, end all equal). The OS-available figure falls between passes, inside ordinary training rounds, in bursts: -858 MB over rounds 101-200, -268 over 501-600, -111 over 601-700, -340 over 1001-1100 (to 35 MB left), then the headroom gate cut the model. Nothing periodic is to blame; the configuration's steady demand plus something bursty in the plain step path exceeds the allowance. Candidates for the bursts, untested: first touch of never-written rows of the per-point buffers as the population grows (pages become resident only when written; 400k capacity, 200k seeded), the background model's warm-up accumulation, the eval caches. Decision with the allowance nearly gone: release configuration back to 120 frames / 330k (proven flat at 1.05 GB), keeping the CPU sweep (exact), the 256-entry authority cache, the memory samples and the events. Next session: run 200 / 400k once more and read `MEMORY EVENTS` together with `MEMORY CURVE` at 100-round resolution to see whether the bursts line up with population growth (first touch) or with warm-up (background accumulation ends at 15 %).

### BUILD 398 (Sept 14, final day): disk-backed frame cache, 30,000 rounds for every scene, early census

Owner's report: a new scan trained only 4,500-ish rounds; small objects crash every time; any background app kills a room run. (1) Rounds: `TrainingBudget.recommended` gave 30,000 only under 8 m (3,500 above), and `ProcessingSupport.plan` took min(plan, pre-pass suggestion) where the pre-pass measures extent from `survey.surfaceBounds` and the plan from `bundle.sceneBounds`; now 30,000 and 120 frames for every scene, the plan no longer takes rounds from the pre-pass, and scenes of 8 m and over use the proven 330k cap instead of an untested 500k. (2) Memory: the frame caches (177-295 MB resident on every room run) are files in tmp, one per frame (luma, 4:2:0 chroma, raw depth samples), read back memory-mapped on a hit; the index is all that stays in memory; stale directories from a crashed run are swept at the first builder of a process; sized from free storage (a quarter, at most 1 GB, off under 2 GB free). The full-size builder's cache stays disabled (382 workaround). (3) The partial census is written every 100 iterations for the first 1,000.

### BUILD 400: best checkpoint on disk; the live preview copies base colour only

(Best checkpoint went into 398's commit.) The early-stop best model was a CPU SplatCloud (330k points with a per-point SH array: about 100 MB, doubled while replaced) held for the whole final level; it is now a raw file of the splat, SH and stats buffers written straight from the shared Metal buffers and read back only if it is shipped. The live preview (every 200 iterations) copied all 48 SH floats a point (63 MB) plus splats and stats, built a degree-3 cloud (about 100 MB) on a background queue and kept the last one, while TrainingPreview reduces every snapshot to degree 0 before drawing; single-slice runs now copy only the base colour (4 MB) and build a degree-0 cloud; snapshots every 500 iterations.

### BUILD 402: no evals inside coarse levels; bounded tile instances; the plan stops inheriting old pre-pass settings

Research workflow (3 Opus + 2 Sonnet) findings, each re-read in the code before use. TRUE and applied: (1) the eval gate had no level check; a coarse-level eval rebuilt every held-out frame at 1,080 px through the full builder (the cache entry is overwritten with the full-size build and fails `cached.renderSize == renderSize`), then `guard frameSupervision.renderSize == renderSize else { continue }` skipped all of them and the call returned nil: 51 evals of a 30,000-round room, about 10 s, no effect on anything recorded; now `!activeIsCoarse`. (2) `ScanProcessingCoordinator` reuses a saved pre-pass by default (`reuseExistingPrePass = true`, and `.buildModel` hard-codes it) and `PrePassResult.currentFormatVersion` is still 1, so `ProcessingSupport.plan`'s field-by-field min with an old `suggestedBudget` capped render size, keyframes and SH degree (and, until 398, rounds) at whatever the older build suggested; now only the memory ceiling (and the cap) come from it. (3) `growInstanceCapacity` had no ceiling and no memory check and is the only pool that grows when a subject fills the frame close up (the object-crash prime suspect; no object census yet): now capped at 64 M (the two-level sort histogram tops out at 67.1 M) and never into the last 384 MB iOS reports; a frame that needs more renders truncated. Also: eval scoring holds at most 4 frames of renders (was one per core, ~140 MB at 1,080 px); authority cache 160 entries. NOT applied yet: compacted dispatch of per-point kernels over drawn points only (Opus estimate 0-78 s, needs a per-view drawn-count measurement first); calibration windows at each level start (3-6 s); warmupStaging / evalStaging on demand (27 + 47 MB at 1,080).

### BUILD 404: points in spatial order every 1,000 iterations

From the research workflow's speed report (Opus), checked in the code: preprocess_backward, regularizer, adam_splat and adam_sh dispatch `count: splatCount` and return early on `tilesTouched[gid] == 0`; Apple GPUs run compute in SIMD groups of 32, so a group costs as much as its busiest lane, and splits and clones are appended at the end, scattering the drawn points through the index order. The report proposed compacted dispatch over a drawn-point list (new kernels, estimated 0 to 78 s). Cheaper version with the same mechanism: every 1,000 iterations the densify pass writes its survivors in Morton order of their centres, through the `source` map every array already moves by (splats and stats on the CPU, SH, both Adam pairs and top-K by the verified GPU gather). Per-point arithmetic is identical; only tie order among equal sort keys and the order of the shared atomic sums change, both already order-noise. Cost about 30 sorts of 330k keys a run. The saving is unmeasured: the stage profile's backward and optimiser times in the next census are the check.
...
### BUILD 410: all 30,000 rounds; reorder timed

scan_20260912_130947 on build 406 (1,396-frame capture, 108 keyframes, 330k): no crash, no cuts, 5 m 25 s training, stopped early at 24,400; held-out 22.04 raw / 22.95 fitted / SSIM 0.6635, trained view 25.85; earlyStopEval 1.39 s (382: 12.4, the coarse-eval skip); densify 30.0 s over 243 passes (382: 11.5 s over 244, different scene; the Morton reorder is a suspect, so it is now timed: `spatialReorderSeconds`); gpuStep 11.07 ms per round (382: 12.57); calibrations at 12,300 chose forward2 and fused blur, rejected SIMD-sum backward (0.047 vs 0.031 s), two-pixel backward, SIMD sort scan (0.00236 vs 0.00222). Footprint 0.9-1.22 GB, 1.95-2.1 GB available. Changes: early stop effectively off (patience 1,000,000 evals) because the owner wants every scan to run 30,000; the end state ships unless the best checkpoint beat it by 0.3 dB.

### BUILD 412: memory readings without the disk query; spatial reorder off

Bedroom scan (scan_20260906, the same capture as 382) on build 408, stopped early at 24,400 like 382: trained view 26.81 (382: 26.85), held-out fitted 20.87 / SSIM 0.6631 (382: 20.87 / 0.663), aligned 21.38 / 0.690; memory peak 961 MB (382: 1,264), per level 498-823 / 609-720 / 691-892 MB, OS available never under 2,037 MB; training 5 m 52 s (382: 5 m 53 s). The eval skip saved 10.9 s (earlyStopEval 1.49 against 12.4) but densify rose 11.5 -> 25.7 s and gpuStep only 306.6 -> 304.5 s. Cause of the densify rise, read in code: `DeviceMemoryFacts.probe()` also calls `freeDiskBytes()` (FileManager `volumeAvailableCapacityForImportantUsage`), and build 392's memory events probe four times inside every densify pass (1,086 events this run): 14.2 s / 972 = about 15 ms a probe. The governor's memory poll (every 50 iterations, about 490 a run) paid the same query since long before 382, about 7 s. Now `probeMemoryOnly()` (os_proc_available_memory only) in the governor poll, the memory events, the densifier probes and instance growth. The Morton reorder (404) is off: no measurable GPU gain.

### BUILD 414: eval every 1,200 iterations

Living room (scan_20260912) on 408: trained view 25.88 (406: 25.85), held-out fitted 22.95 / SSIM 0.6629 (406: 22.95 / 0.6635), aligned 23.24 / 0.676; peak 948 MB (406: about 1,224), per level 509-831 / 620-735 / 744-918 MB, OS available never under 2,018 MB; 5 m 22 s (406: 5 m 25 s), densify 25.5 s. With early stopping off, evals feed only the curve and a checkpoint that must beat the end state by 0.3 dB, so the interval goes 400 -> 1,200 (about 7 evals in the 1,080-px level instead of 22). A45 (backward p/q accumulation) left alone: a rewrite of the SIMD-summed backward with no on-device check before the deadline.
