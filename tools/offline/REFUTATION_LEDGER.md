...
## Exposure (2026-09-11, build 276 notes)

| id | finding | verdict | what was checked |
|---|---|---|---|
| A38 | per-frame exposure correction is inert | **SPLIT** | exp_a26_a38.py, reproduced by the checker: after 37 visits the GAIN lands at 0.9988-1.0086 (inert) but the BIAS reaches +0.005 to +0.031 (up to 62% of its clamp) and adds +0.46 dB to trained frames only. A38's 1.9% came from a synthetic render with no brightness offset. On 244-272 six held-out frames were trained, so they carried learned exposures into the held-out score |
| A26 | the capture's exposure varies | CONFIRMED, not measurable | shutter spans 0.74 stops but ISO is absent on 868/868 frames (the capture had no camera device, fixed 270), so duration x gain cannot be formed. Frames 0-15 (all 1/60 s): photos 6-16% brighter than the model's render, above 1.1 on 12/16; white balance drifts up to 12% in red. A shutter-only prior costs 7.1 dB: never ship one |
| EXP-LS | the held-out least-squares exposure fit reads exposure | REFUTED (measured) | the fitted gain is 1 - cov(render, error)/var(render): corr with raw PSNR +0.93, with blur +0.01; the bias clamp binds on 16/16 frames. Checkpoint selection runs on this fitted score. Stage 1 (276) records raw beside it; switching selection to raw is a device A/B |
| EXP-FIX | per-frame gain from the photo/render mean-brightness ratio, after warm-up, re-centred, bias 0 | OPEN (device A/B) | design checked against the code; must use NEW tuning fields (exposureGainRange also clamps the eval fit). +0.185 dB on fixed renders of frames 0-15; held-out effect not measured. Baseline must be 274's clean held-out set |
| HDR | the HDR check reads the video format before it is chosen | OPEN (capture) | ARCaptureService: isVideoHDRSupported is read before preferredVideoFormat() is assigned. Needs a new scan to test |
